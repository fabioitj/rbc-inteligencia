...
const uri =
  "mongodb+srv://rbcunivali:oQNsQavxqxzWbLwv@rbc.oc3iv9p.mongodb.net/?retryWrites=true&w=majority&appName=RBC";
...
app.get("/match/:id", async (req: Request, res: Response) => {
  try {
    ...
    const partidasParaComparar = partidas.filter(
      (partida) => partida._id.toString() !== partidaBase._id.toString(),
    );

    const partidasComSimilaridade = partidasParaComparar.map(
      (partida: Match) => ({
        partida,
        similaridade: calculateSimilarity(
          <MatchModel>{
